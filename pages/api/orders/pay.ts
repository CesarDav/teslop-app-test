...
const payOrder = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    if (!dbOrder) {
        await db.disconnect();
        return res.status(401).json({ message: 'Order does not exist in our database ' });
    }

    if (dbOrder.total !== Number(data.purchase_units[0].amount.value)) {
        await db.disconnect();
        return res.status(401).json({ message: 'The paypal amounts and our order are not the same. ' });
